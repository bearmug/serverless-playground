...
* **native lambdas for Java8 and Kotlin**
    Micronaut-based lambdas with [GraalVM](https://www.graalvm.org/docs/why-graal/) custom runtime. It promises to solve
    cold start issues and keep warm timings similar to **java8**. [Here is Micronaut](https://micronaut-projects.github.io/micronaut-aws/latest/guide/#_custom_graalvm_native_runtimes)
    build instruction.
...
### Cold start test
AWS Lambda deployment guarantees (within our scenario) that first lambda call will experience "cold start". Therefore,
the "cold start" benchmarking strategy is:
* deploy lambdas set
* load test each of them with 10 seconds 1-virtual user run ([see run body here](../config/load-test/load-test-simple.js))
* repeat deploy-benchmark iteration 10 times
You can run the same [script](../config/load-test/aws-deploy-cold-call.sh) with:
```bash
./config/load-test/aws-deploy-cold-call.sh
```

### Warm runs test
Lambda stay "warm" (not having init phase) some time after each call. Warm runs test uses this fact, it's scenario
...
- run the [same](../config/load-test/load-test-simple.js) load test body 10 times
Warm runs load test could be triggered by [script](../config/load-test/aws-deploy-warm-call.sh) call:
```bash
./config/load-test/aws-deploy-warm-call.sh
```

> Please remember to modify [cold](../config/load-test/aws-deploy-cold-call.sh) and 
> [warm](../config/load-test/aws-deploy-warm-call.sh) scripts with actual lambda URLs. You may find them with command:
```bash
# run in project root
