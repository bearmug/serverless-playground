...
tasks.register<Exec>("gcpStartLocal") {
    group = "serverless"
    dependsOn("gcpAssemble")
    commandLine("bash", "-c",
...
tasks.register<Exec>("gcpStopLocal") {
    group = "serverless"
    commandLine("bash", "-c",
...
////AWS tasks=====================================================================
tasks.register<Exec>("awsLambdaAssemble"){
    group = "serverless"
    inputs.files(
            project.fileTree("src"),
...
tasks.register<Exec>("awsLambdaDeploy"){
    group = "serverless"
    dependsOn("build", "awsLambdaAssemble")
    commandLine("bash", "-c", "serverless deploy")
}

tasks.register<Exec>("awsLambdaRemove") {
    group = "serverless"
    commandLine("bash", "-c", "serverless remove")
}

tasks.register<Exec>("awsLambdaStatsJava") {
    group = "serverless"
...
tasks.register<Exec>("awsLambdaStatsGraal") {
    group = "serverless"
...
tasks.register<Exec>("awsLambdaStatsNode") {
    group = "serverless"
